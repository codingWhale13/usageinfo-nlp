...
def benchmark(generator_parameters: dict):
    ...
    for review_id, review_data in results.items():
        review = review_data["results"]
        iterations = review_data["iterations"]
    ...
    pd.DataFrame.from_records(stats_df).to_csv(f"pg_hp_test_{parameter_id}.csv")
    pd.DataFrame.from_records(list(review_stats.values())).to_csv(
        f"reviews_pg_hp_test_{parameter_id}.csv"
    )
...
with SustainabilityTracker() as sustainability_tracker:
    ...
    sweep(
        "minimum_probability",
        [0.1, 0.05, 0.02, 0.01, 0.005, 0.002, 0.001, 0.0001],
