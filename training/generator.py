...
class Generator:
    ...
    def _get_output_with_probs(self, batch, forced_decoder_ids=[], current_probability=1.0):
        ...
        # print(probs)
        best_token_ids = torch.argsort(probs, descending=True)[
            :num_token_options
        ]
        ...
        for token_id in best_token_ids[:i]:
            yield {"forced_decoder_ids": forced_decoder_ids + [(max_new_tokens, int(token_id))], "current_probability": current_probability * probs[token_id]}
    ...
    def generate_usage_options2(self, batch) -> None:
        ...
        while generation_queue:
            next_generations = self._get_output_with_probs(batch=batch, **generation_queue.pop(0))
        ...
        decoded_results = self.tokenizer.batch_decode([[token_id for _, token_id in result["forced_decoder_ids"]] for result in results])
        decoded_results_with_probs = [(float(result["current_probability"]), output) for result, output in zip(results, decoded_results)]
        print(model_inputs)
        decoded_results_with_probs = sorted(decoded_results_with_probs, key=lambda x: x[0], reverse=True)
