...
def parse_args():
    ...
    argparser.add_argument("run_names", nargs="+", help="run_names of the runs you want to score")
...
def read_dataframes(run_names: list[str], file_name: str) -> pd.DataFrame:
    ...
    for run_name in run_names:
        csv_file_path = get_model_dir_file_path(run_name + "-active_learning_dir",file_name)
...
def read_data_frames_over_all_iteration(run_names: list[str], base_file_name: str, count_acquired_training_reviews=False) -> pd.DataFrame:
    all_iterations_df = []
    for iteration in range(0, MAX_ACTIVE_LEARNING_ITERATIONS):
        ...
        if count_acquired_training_reviews:
            for run_name in run_names:
                df.loc[df["run_name"] == run_name, "acquired_training_reviews"] = len(df[df["run_name"] == run_name] )
...
def plot_run_scores(run_names: list[str]) -> None:
    ...
    print(all_runs_df)
    for metric in ["validation_loss", "harmonic_balanced"]: #"custom_weighted_mean_f1_mean"
        plt.clf()
        sns.lineplot(data=all_runs_df, x="acquired_training_reviews", y=metric, hue="run_name", markers=True, style="run_name")
...
def plot_run_datasets_input_word_count(run_names: list[str]) -> None:
    ...
    for iteration in range(0, 5):
        ...
        for run_name in run_names:
            df.loc[df["run_name"] == run_name, "acquired_training_reviews"] = len(df[df["run_name"] == run_name] ) # ["acquired_training_reviews"] = 
        all_iterations_df.append(df)
    ...
    plt.clf()
    sns.lineplot(data=all_iterations_df, x="acquired_training_reviews", y="input_word_count", hue="run_name", markers=True, dashes=False, style="run_name")
    plt.axhline(y=mean_input_word_count)
    plt.savefig(f"{run_names_report_dir}/input_word_count{'_'.join(run_names)}.png")

    mean_usage_options_word_count = training_dataset_df[training_dataset_df["has_usage_options"] == True]["usage_options_word_count"].mean()
    plt.clf()
    sns.lineplot(data=all_iterations_df[all_iterations_df["has_usage_options"] == True], x="acquired_training_reviews", y="usage_options_word_count", hue="run_name", markers=True, dashes=False, style="run_name")
    plt.axhline(y=mean_usage_options_word_count)
    plt.savefig(f"{run_names_report_dir}/usage_options_word_count{'_'.join(run_names)}.png")

    mean_usage_optons_count = training_dataset_df[training_dataset_df["has_usage_options"] == True]["usage_options_count"].mean()
    plt.clf()
    sns.lineplot(data=all_iterations_df[all_iterations_df["has_usage_options"] == True], x="acquired_training_reviews", y="usage_options_count", hue="run_name", markers=True, dashes=False, style="run_name")
...
def plot_run_acquisition_function_score(run_names: list[str]) -> None:
    ...
    plt.clf()
    sns.lineplot(data=df, x="iteration", y="entropy", hue="run_name", markers=True, style="run_name")
...
def plot_run_datasets_usage_option_percentage(run_names: list[str]) -> None:
    ...
    for iteration in range(0, 5):
        df = read_dataframes(run_names, f"training_dataset_iteration_{iteration}.csv")
        frac = df.groupby(["run_name"])["has_usage_options"].value_counts(normalize=True).unstack(-1).fillna(0).add_prefix('percentage_has_usage_options_')
        frac["iteration"] = iteration
        frac = frac.reset_index()
        print(frac)
        for run_name in run_names:
            frac.loc[frac["run_name"] == run_name, "acquired_training_reviews"] = len(df[df["run_name"] == run_name] ) # ["acquired_training_reviews"] = 
        all_iterations_df.append(frac)
    all_iterations_df = pd.concat(all_iterations_df)
    print(all_iterations_df)

    training_dataset_df = get_training_dataset_df()
    usage_options_percentage = len(training_dataset_df[training_dataset_df["has_usage_options"] == True]) / len(training_dataset_df)
    plt.clf()
    sns.lineplot(data=all_iterations_df, x="acquired_training_reviews", y="percentage_has_usage_options_True", hue="run_name", markers=True, dashes=False, style="run_name")
    plt.axhline(y=usage_options_percentage)
    plt.savefig(f"{run_names_report_dir}/percentage_has_usage_options{'_'.join(run_names)}.png")

if __name__ == "__main__":
    args = parse_args()

    run_names_report_dir = absolute_path_from_this_file_to(f"score_reports/{'_'.join(args.run_names)}")
