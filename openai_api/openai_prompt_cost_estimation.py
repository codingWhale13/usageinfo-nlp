...
def estimate_price_per_review(prompt, model):
    ...
    total_word_count = prompt_word_count + MEAN_WORDS_PER_REVIEW + MEAN_WORDS_PER_OPENAI_USAGE_OPTIONS_OUTPUT
...
def parse_args():
    ...
    arg_parser.add_argument(
        ...
        help="OpenAI model. Influences price")
...
def main():
    ...
    with open(prompts_file_name, 'r+') as prompts_file:       
        ...
        for prompt in prompts_json:
            estimated_price_per_review = estimate_price_per_review(prompt, openai_model_used)
