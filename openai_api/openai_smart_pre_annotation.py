...
def generate_samples(review: json, model: str = "text-davinci-003", prompt: str = None, n: int=1):
    return [pre_label_with_logprobs(review, model=model,prompt=prompt) for _ in range(n)]
...
def pre_label_with_dynamic_samplling( review: json, model: str = "text-davinci-003", prompt: str = None):
    ...
    if len(output_types_counter.keys()) == 1:
        ...
        samples = samples + generate_samples(review, model=model, prompt=prompt, n=SECOND_STEP_SAMPLES)
        output_types_counter = count_output_types(samples)
        if output_types_counter[True] >= FIRST_STEP_SAMPLES + SECOND_STEP_SAMPLES - 1:
            print(f"3/4 for true")
            return select_most_likely_sample(samples, desired_output_type=True, review=review)
        elif output_types_counter[False] >= FIRST_STEP_SAMPLES + SECOND_STEP_SAMPLES - 1:
            print(f"3/4 for false")
            return select_most_likely_sample(samples, desired_output_type=False, review=review)
        else:
            samples = samples + generate_samples(review, model=model, prompt=prompt, n=1)
