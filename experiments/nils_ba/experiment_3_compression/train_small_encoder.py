...
from training.encoder_model import EncoderModel
...
# teacher = model.model.encoder.block[:-1]  # all of the encoder except the last layer
student = model.model.encoder.block[
    :2
]  # similar architecture but only two instead of 12 blocks (taking the first two is arbitrary)
...
# let teacher generate training data ("annotate")
test_dataset = model.test_reviews
label_id = f"model-{wandb.run.name}-teacher"
