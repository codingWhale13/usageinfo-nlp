...
const REVIEWS =
  process.env.NODE_ENV === "production"
    ...
    : {
        ...
        review_bodies: [
          "I like this axe for chopping wood and digging a fire pit.",
          "Hello world wow 1",
        ],
      };
...
class MTurkReview extends Component {
  constructor(props) {
    ...
    for (let index = 0; index < REVIEWS.review_bodies.length; index++) {
      ...
      const allUsageOptions = REVIEWS.metadata[index][CUSTOM_USAGE_OPTIONS];

      this.state.label[PREDICTED_USAGE_OPTIONS].push(
        allUsageOptions.map((label) => {
          return {
            [PREDICTED_USAGE_OPTION_LABEL]: label,
  ...
  updateInspectionTime = (callback) => {
    ...
    inspectionTimes[this.state.reviewIndex] =
      inspectionTimes[this.state.reviewIndex] + time;
  ...
  render() {
    ...
    const predictedUsageOptions =
      this.state.label[PREDICTED_USAGE_OPTIONS][index];
    const customUsageOptions = this.state.label[CUSTOM_USAGE_OPTIONS][index];
    const containsMoreUsageOptions =
      this.state.label[CONTAINS_MORE_USAGE_OPTIONS][index];
    const isLastReview =
      this.state.reviewIndex === this.state.reviews.length - 1;
          ...
          {JSON.stringify(
            {
              ...
              [CONTAINS_MORE_USAGE_OPTIONS]:
                this.state.label[CONTAINS_MORE_USAGE_OPTIONS],
